...
| 步骤 | 状态 | 说明 |
|---|---|---|
| 能力分类 | mock(关键词匹配) | 例:含"手指"→ `human_hand`,含"运镜"→ `camera_motion` |
| 检查项 + 测试变量生成 | **真实 LLM** | 默认 `deepseek-v4-pro`,可换 |
| 测试用例生成 | **真实 LLM** | 默认 `deepseek-chat`,可换;每批注入 40 个具体场景标签防"空话" |
| 难度评分 | **真实** | 启发式规则(长度 / 子句数 / 动词密度) |
| 自动质检 | 跳过 | v0.7 接入 |
| 评测员手册 | **真实** | 从已生成数据渲染 |
| 任务状态持久化 | mock(内存) | v0.7 接入 SQLite |
