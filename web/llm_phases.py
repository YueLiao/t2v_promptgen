...
_PROMPTS_SYSTEM = """你是 T2V 评测 prompt 设计专家。给定一组 SL2(失败模式)和 axes(测试变量),生成 N 条 prompt。

严格规则:
- 每条 prompt 必须明确测试至少 1 个 SL2 + 设置至少所有 axes 的具体值
...
要求多样化覆盖不同 SL2 × axes 组合,不要重复。难度 medium 60% / hard 40%,is_stress 约 30%。
"""
        try:
            resp = client.generate(
                messages=[{"role": "user", "content": user_msg}],
